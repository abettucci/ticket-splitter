...
const EmptyState = ({ onCreate }: { onCreate: () => void }) => (
  <section className="relative overflow-hidden rounded-[2rem] bg-[#0e1c2f] px-6 py-12 text-white shadow-2xl shadow-slate-900/20 sm:px-12 sm:py-16">
    <div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-[#fe6f5e]/30 blur-3xl" />
    <div className="absolute -bottom-24 left-16 h-52 w-52 rounded-full bg-[#24b9a9]/20 blur-3xl" />
    <div className="relative max-w-xl">
      <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-[#a8eee2]">
        <WalletCards className="h-3.5 w-3.5" /> SplitBot web
      </p>
      <h2 className="font-serif text-4xl leading-[1.02] tracking-tight sm:text-5xl">La próxima cuenta empieza acá.</h2>
      <p className="mt-5 max-w-md text-base leading-relaxed text-slate-300">
        Cargá los gastos, vemos quién adelantó qué y dejamos las transferencias claras. Sin cuenta, sin vueltas.
      </p>
      <Button onClick={onCreate} className="mt-8 h-12 rounded-full bg-[#fe6f5e] px-6 text-base font-bold text-white hover:bg-[#ff8b79]">
...
export default function Splitter() {
  ...
  if (!activeDivision) {
    return (
      <main className="min-h-screen bg-[#f5f1e9] text-[#0e1c2f]">
        <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
          <header className="mb-10 flex items-center justify-between">
            <Link to="/" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 transition hover:text-[#0e1c2f]">
              <ArrowLeft className="h-4 w-4" /> Volver al inicio
            </Link>
            <span className="font-serif text-xl font-bold tracking-tight">SplitBot</span>
          </header>

          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-[#e65a4a]">Sin registro · guardado en este navegador</p>
              <h1 className="font-serif text-4xl tracking-tight sm:text-5xl">Mis divisiones</h1>
            </div>
            {divisions.length > 0 && (
              <Button onClick={createNewDivision} className="h-11 rounded-full bg-[#0e1c2f] px-5 font-bold hover:bg-[#1c3555]">
          ...
          {divisions.length === 0 ? (
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {divisions.map((division) => {
                ...
                return (
                  <article key={division.id} className="group rounded-3xl border border-[#ddd6c9] bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-900/5">
                    <div className="mb-8 flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <p className="truncate font-serif text-2xl font-bold tracking-tight">{division.name}</p>
                      ...
                      <span className="rounded-full bg-[#e4f4f0] px-2.5 py-1 text-xs font-bold text-[#08796c]">ARS</span>
                    </div>
                    <p className="font-serif text-3xl tracking-tight">{formatARS(total)}</p>
                    <p className="mt-1 text-sm text-slate-500">
                    ...
                    <div className="mt-7 flex items-center gap-2">
                      <Button onClick={() => setActiveId(division.id)} className="h-10 flex-1 rounded-full bg-[#0e1c2f] font-bold hover:bg-[#1c3555]">
                        Abrir <ChevronRight className="h-4 w-4" />
                      </Button>
                      <Button aria-label={`Duplicar ${division.name}`} onClick={() => duplicateDivision(division)} size="icon" variant="outline" className="rounded-full border-[#ddd6c9]">
  ...
  return (
    <main className="min-h-screen bg-[#f5f1e9] text-[#0e1c2f]">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-7 flex flex-wrap items-center gap-3 border-b border-[#ddd6c9] pb-5">
          <Button onClick={() => setActiveId(null)} variant="ghost" className="-ml-3 rounded-full text-slate-600 hover:bg-white hover:text-[#0e1c2f]">
            <ArrowLeft className="h-4 w-4" /> Mis divisiones
          </Button>
          <div className="hidden h-5 w-px bg-[#ddd6c9] sm:block" />
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <PencilLine className="h-4 w-4 shrink-0 text-[#e65a4a]" />
            <input
              ...
              className="w-full min-w-0 bg-transparent font-serif text-2xl font-bold tracking-tight outline-none placeholder:text-slate-400 sm:text-3xl"
            />
          </div>
          <div className="ml-auto flex items-center gap-2">
            <Button onClick={() => exportDivision(activeDivision)} variant="outline" className="rounded-full border-[#cfc5b6] bg-white text-slate-700 hover:bg-[#fffaf1]">
              <Download className="h-4 w-4" /><span className="hidden sm:inline">Exportar</span>
            </Button>
            <Button onClick={shareSummary} disabled={!calculation || calculation.issues.length > 0} className="rounded-full bg-[#e65a4a] font-bold hover:bg-[#c94638]">
        ...
        <div className="mb-7 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-[#ddd6c9] bg-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Total cargado</p>
            <p className="mt-1 font-serif text-2xl">{formatARS(totalExpenses)}</p>
          </div>
          <div className="rounded-2xl border border-[#ddd6c9] bg-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Personas</p>
            <p className="mt-1 font-serif text-2xl">{activeDivision.participants.length}</p>
          </div>
          <div className="rounded-2xl border border-[#ddd6c9] bg-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Estado</p>
            <p className="mt-1 font-serif text-2xl">{hasPayments ? `${activeDivision.payments.length} pago${activeDivision.payments.length === 1 ? "" : "s"}` : "Al día"}</p>
          </div>
        </div>

        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="space-y-6">
            <section className="rounded-3xl border border-[#ddd6c9] bg-white p-5 shadow-sm sm:p-7">
              <div className="mb-5 flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#08796c]">Paso 1</p>
                  <h2 className="mt-1 font-serif text-2xl tracking-tight">¿Quiénes están?</h2>
                </div>
                <UsersRound className="h-6 w-6 text-[#24b9a9]" />
              </div>
              <div className="flex flex-wrap gap-2">
                {activeDivision.participants.map((participant) => (
                  <div key={participant.id} className="inline-flex items-center gap-2 rounded-full bg-[#e4f4f0] py-1.5 pl-3 pr-1.5 text-sm font-semibold text-[#075d54]">
                    {participant.name}
                    <button onClick={() => deleteParticipant(participant.id)} aria-label={`Quitar a ${participant.name}`} className="rounded-full p-1 text-[#08796c] transition hover:bg-white hover:text-red-600">
              ...
              <div className="mt-4 flex gap-2">
                <input
                  ...
                  className="h-10 min-w-0 flex-1 rounded-xl border border-[#d8d1c5] bg-[#fffdf9] px-3 text-sm outline-none transition focus:border-[#24b9a9] focus:ring-2 focus:ring-[#24b9a9]/15"
                />
                <Button onClick={addParticipant} variant="outline" className="h-10 rounded-xl border-[#cfc5b6] bg-white font-bold hover:bg-[#e4f4f0]">
            ...
            <section className="rounded-3xl border border-[#ddd6c9] bg-white p-5 shadow-sm sm:p-7">
              <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#e65a4a]">Paso 2</p>
                  <h2 className="mt-1 font-serif text-2xl tracking-tight">Cargá los gastos</h2>
                </div>
                <Button
                  onClick={() => {
                  ...
                  className="rounded-full bg-[#0e1c2f] font-bold hover:bg-[#1c3555]"
                >
              ...
              {activeDivision.expenses.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-[#cfc5b6] bg-[#fffaf1] px-5 py-8 text-center">
                  <ReceiptText className="mx-auto h-7 w-7 text-[#e65a4a]" />
...
function ExpenseEditor({
  ...
  return (
    <article className="rounded-2xl border border-[#e2dcd2] bg-[#fffdf9] p-4 sm:p-5">
      <div className="flex items-start gap-3">
        <span className="mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#0e1c2f] text-xs font-bold text-white">{index + 1}</span>
        <div className="grid min-w-0 flex-1 gap-3 sm:grid-cols-[minmax(0,1fr)_150px]">
          <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500">
            ...
            <input
              ...
              className="mt-1.5 h-10 w-full rounded-xl border border-[#d8d1c5] bg-white px-3 text-sm font-medium normal-case tracking-normal text-[#0e1c2f] outline-none focus:border-[#24b9a9] focus:ring-2 focus:ring-[#24b9a9]/15"
            />
          </label>
          <label className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500">
            Total ARS
      ...
      <div className="mt-5 grid gap-5 border-t border-[#e7e0d7] pt-5 lg:grid-cols-2">
        <div>
            ...
            {expense.paidBy.map((payer) => (
              <div key={payer.participantId} className="flex gap-2">
                <select
                  ...
                  className="h-9 min-w-0 flex-1 rounded-lg border border-[#d8d1c5] bg-white px-2 text-sm outline-none focus:border-[#24b9a9]"
                >
                ...
                <input
                  ...
                  placeholder="0,00"
                  className="h-9 w-28 rounded-lg border border-[#d8d1c5] bg-white px-2 text-right text-sm outline-none focus:border-[#24b9a9]"
                />
          ...
          {expense.paidBy.length < division.participants.length && (
            <button
              ...
              className="mt-2 inline-flex items-center gap-1 text-sm font-bold text-[#08796c] hover:text-[#06554d]"
            >
              <Plus className="h-3.5 w-3.5" /> Sumar pagador
            </button>
          )}
          <p className={`mt-2 text-xs font-medium ${paidTotal === expense.amountCents && expense.amountCents > 0 ? "text-[#08796c]" : "text-[#c75a22]"}`}>
        ...
        <div>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="inline-flex rounded-lg bg-[#eee9df] p-0.5 text-xs font-bold">
              <button onClick={() => setMode("equal")} className={`rounded-md px-2 py-1 ${expense.splitMode === "equal" ? "bg-white text-[#0e1c2f] shadow-sm" : "text-slate-500"}`}>Igual</button>
              <button onClick={() => setMode("exact")} className={`rounded-md px-2 py-1 ${expense.splitMode === "exact" ? "bg-white text-[#0e1c2f] shadow-sm" : "text-slate-500"}`}>Exacto</button>
            </div>
          </div>
            ...
            {division.participants.map((participant) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-2.5 py-1 text-xs font-bold transition ${selected ? "bg-[#0e1c2f] text-white" : "bg-[#eee9df] text-slate-500 hover:bg-[#ddd6c9]"}`}
                >
              ...
              {selectedParticipants.map((participant) => (
                <label key={participant.id} className="flex items-center gap-2 text-sm text-slate-600">
                  ...
                  <input
                    ...
                    placeholder="0,00"
                    className="h-8 w-28 rounded-lg border border-[#d8d1c5] bg-white px-2 text-right text-sm outline-none focus:border-[#24b9a9]"
                  />
                </label>
              ))}
            </div>
...
function BalancePanel({
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-[#0e1c2f] text-white shadow-xl shadow-slate-900/15">
      <div className="border-b border-white/10 px-5 py-5 sm:px-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#a8eee2]">Paso 3</p>
            <h2 className="mt-1 font-serif text-2xl tracking-tight">El balance</h2>
          </div>
          <Landmark className="h-6 w-6 text-[#fe8878]" />
        </div>
      </div>

      {calculation.issues.length > 0 ? (
        <div className="px-5 py-6 sm:px-6">
          <p className="text-sm font-semibold text-[#ffd0b0]">Faltan algunos datos antes de calcular.</p>
          <ul className="mt-4 space-y-2 text-sm leading-relaxed text-slate-300">
            {calculation.issues.map((issue) => <li key={issue} className="flex gap-2"><span className="text-[#fe8878]">•</span>{issue}</li>)}
          </ul>
        </div>
      ) : (
        <div className="px-5 py-6 sm:px-6">
          ...
          <div className="mt-5 space-y-3">
            {calculation.settlements.length === 0 ? (
              <div className="rounded-2xl bg-[#15314d] px-4 py-5 text-center">
                <Check className="mx-auto h-6 w-6 text-[#70dccd]" />
                <p className="mt-2 font-serif text-xl">Todo saldado</p>
                <p className="mt-1 text-sm text-slate-300">No quedan transferencias pendientes.</p>
              </div>
            ) : calculation.settlements.map((settlement) => (
              <div key={`${settlement.debtorParticipantId}-${settlement.creditorParticipantId}-${settlement.amountCents}`} className="rounded-2xl bg-[#15314d] p-4">
                <div className="flex gap-2 text-sm leading-snug">
                  ...
                  <span className="font-bold text-[#a8eee2]">{formatARS(settlement.amountCents)}</span>
                  <span className="text-slate-400">a</span>
                  <span className="font-bold text-white">{participantName(division, settlement.creditorParticipantId)}</span>
                </div>
                <Button onClick={() => onMarkPaid(settlement)} className="mt-3 h-9 w-full rounded-xl bg-[#24b9a9] text-xs font-bold text-[#062d2a] hover:bg-[#70dccd]">
              ...
              <div className="mt-3 space-y-2">
                {division.payments.map((payment) => (
                  <div key={payment.id} className="flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2.5 text-xs text-slate-300">
                    <Check className="h-4 w-4 shrink-0 text-[#70dccd]" />
              ...
              {division.participants.map((participant) => {
                ...
                return (
                  <div key={participant.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className={balance > 0 ? "font-bold text-[#a8eee2]" : balance < 0 ? "font-bold text-[#ffd0b0]" : "font-bold text-slate-400"}>
